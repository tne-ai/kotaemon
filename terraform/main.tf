# Main Terraform Configuration for Kotaemon RAG Application
...
resource "aws_efs_file_system" "app_data" {
  creation_token   = "${var.project_name}-${var.environment}-app-data"
...
resource "aws_efs_access_point" "app_data" {
  ...
  tags = {
    Name = "${var.project_name}-${var.environment}-efs-access-point"
  }
}
...
resource "aws_secretsmanager_secret" "openai_api_key" {
  ...
  description = "OpenAI API key for Kotaemon application"
...
resource "aws_secretsmanager_secret" "cohere_api_key" {
  ...
  description = "Cohere API key for Kotaemon application"
...
resource "aws_secretsmanager_secret_version" "cohere_api_key" {
  secret_id     = aws_secretsmanager_secret.cohere_api_key.id
  secret_string = var.cohere_api_key
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "KH_FEATURE_USER_MANAGEMENT"
          value = "true"
        }
      ]
      ...
      secrets = [
        ...
        {
          name      = "COHERE_API_KEY"
          valueFrom = aws_secretsmanager_secret.cohere_api_key.arn
        }
      ]
      ...
      mountPoints = [
        {
          ...
          containerPath = "/app/ktem_app_data"
          readOnly      = false
        }
      ]
